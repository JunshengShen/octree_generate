...
#include <iostream>
using namespace std;
bool overlap1(coor tri1, coor tri2, coor tri3, float voxel_xn, float voxel_xp, float voxel_yn, float voxel_yp, float voxel_zn, float voxel_zp)
{
	...
	for (int i = 0; i < polygon.size(); i++)
	{
		...
		if (polygon[location_temp[0]].x<voxel_xn&&polygon[location_temp[1]].x>voxel_xn)
		{
			polygon[i].x = voxel_xn;
			polygon[i].y = polygon[i].y + (voxel_xn - polygon[i].x)
				*(polygon[location_temp[1]].y - polygon[i].y) / (polygon[location_temp[1]].x - polygon[i].x);
			polygon[i].z = polygon[i].z + (voxel_xn - polygon[i].x)
				*(polygon[location_temp[1]].z - polygon[i].z) / (polygon[location_temp[1]].x - polygon[i].x);
		}
		else if (polygon[location_temp[0]].x>voxel_xn&&polygon[location_temp[1]].x < voxel_xn)
		{
			polygon[i].x = voxel_xn;
			polygon[i].y = polygon[i].y + (voxel_xn - polygon[i].x)
